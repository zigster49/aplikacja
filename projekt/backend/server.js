...
function RequireAdmin(req, res, next) {
  if (req.session && req.session.userId && req.session.role === "admin") {
    ...
    res
      .status(403)
      .send(
        "Brak dostępu. Ta strona jest przeznaczona tylko dla administratorów.",
      );
  }
}
...
apiRouter.get("/quizzes/:id", async (req, res) => {
  db.get(
    "SELECT id, title, data FROM quizzes WHERE id = ?",
    [req.params.id],
    (err, row) => {
      if (err) {
  ...
  db.run(query, values, function (err) {
    if (err) {
      ...
      if (err.message.includes("UNIQUE constraint failed")) {
        return res
          .status(400)
          .json({ message: "Podany login lub adres e-mail jest już zajęty." });
      }

      return res
        .status(500)
        .json({ message: "Błąd serwera podczas aktualizacji danych." });
    }
...
apiRouter.get("/users/:id", RequireAdmin, (req, res) => {
  ...
  db.get(query, [req.params.id], (err, user) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Błąd pobierania informacji o użytkowniku" });
    }
    if (!user) {
      return res
        .status(404)
...
apiRouter.get("/users", RequireAdmin, (req, res) => {
  ...
  db.all(query, [], (err, rows) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Błąd pobierania listy użytkowników" });
    }
    ...
    async (err, row) => {
      ...
      if (row) {
        return res
          .status(409)
          .json({
            message: "Użytkownik o podanym loginie lub e-mailu już istnieje.",
          });
      }
      ...
      try {
        ...
        db.run(
          ...
          function (err) {
            ...
            return res
              .status(201)
              .json({ message: "Użytkownik został pomyślnie utworzony." });
          },
        );
      } catch (error) {
